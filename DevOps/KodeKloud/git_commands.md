...
`git log --graph --decorate`: This command displays the commit history in a graphical format, showing the branching and merging of commits. The `--decorate` option adds branch and tag names to the output, making it easier to identify specific commits and branches.